...
static void opticalEncoderCallback( const msg::optical_encoder::ConstPtr& msg ) {
  ...
  if(optical_encoder_instance == 14) {
    ros::spinOnce();
    ...
    scan.angle_min = 0; // Start angle of scan [rad]
    scan.angle_max = 2 * 3.1415926;   // End angle of scan [rad]
    ...
    scan.angle_increment = (scan.angle_max - scan.angle_min) / (double) callback_instance;

    // Time between measurements [seconds]
    //scan.time_increment = (1 / LASER_FREQUENCY) / (NUM_READINGS);
    scan.time_increment = (encoder_end_time.toSec() - 
                          encoder_start_time.toSec()) / (double)
			  callback_instance;
    
    // Range and intensity data during that one second of collection
    scan.ranges.resize(callback_instance);
    for(unsigned int i = 0; i < callback_instance; ++i)
      scan.ranges[i] = sensor_ranges[i];
    
    std::cout << "Start Angle: " << scan.angle_min << "\t| End Angle: " <<
    scan.angle_max << "\t| Avg Speed: " << encoder_avg_angular_velocity<< std::endl;
...
static float calculateEndAngle() {
  return calculateStartAngle() + encoder_avg_angular_velocity;
  //ros::Duration timeDiff = teraranger_end_time - encoder_end_time;
  //float timeDiffSec = timeDiff.toSec();
  //float angleDiff = encoder_avg_angular_velocity * timeDiffSec;

  //return (encoder_angle_end + angleDiff);
  //float startAngle = calculateStartAngle();
  //float timeDiffSec = (teraranger_end_time.toSec() - teraranger_start_time.toSec() );
...
int main( int argc, char ** argv ) {
  ...
  //ros::Subscriber optical_encoder_sub = n.subscribe("optical_encoder",
  //  NUM_READINGS, opticalEncoderCallback );
  std::cout << "Initialized node handle" << std::endl;
  scan_pub = n.advertise<sensor_msgs::LaserScan>("laser_scan", 1);
  std::cout << "Initialized scan_pub" << std::endl;
  teraranger_sub = n.subscribe("terarangerone", NUM_READINGS,
    terarangeroneCallback );
  std::cout << "Initialized teraranger_sub" << std::endl;
    
  optical_encoder_sub = n.subscribe("optical_encoder",
    NUM_READINGS, opticalEncoderCallback );
  std::cout << "Initialized optical_encoder_sub" << std::endl;
  ros::spinOnce();
